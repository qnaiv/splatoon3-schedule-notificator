...
const API_CONFIG = {
  ...
  scheduleEndpoint: import.meta.env.PROD
    ? './api/schedule.json'
    : '/api/schedule.json',
  currentEndpoint: import.meta.env.PROD
    ? './api/current.json'
    : '/api/current.json',
};

export const useSchedule = () => {
  const [scheduleData, setScheduleData] = useState<Spla3ApiResponse | null>(
    null
  );
  ...
  // 全データから現在のマッチを抽出
  const getCurrentMatchesFromFull = useCallback(
  ...
  const updateScheduleData = useCallback(async () => {
    try {
      ...
      if (!result || typeof result !== 'object') {
        throw new Error(
          'Invalid API response: result object is missing or invalid'
        );
      }
  ...
  // 特定時間帯のマッチ取得
  const getMatchesInTimeRange = useCallback(
    (startTime: Date, endTime: Date): ScheduleMatch[] => {
  ...
  // ルール別マッチ取得
  const getMatchesByRule = useCallback(
    (ruleName: string): ScheduleMatch[] => {
      if (!scheduleData) return [];
  ...
  // ステージ別マッチ取得
  const getMatchesByStage = useCallback(
    (stageId: string): ScheduleMatch[] => {
      if (!scheduleData) return [];
