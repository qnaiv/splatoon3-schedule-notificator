...
export const useSettings = () => {
  ...
  const saveSettingsToDB = useCallback(
    async (newSettings: UserSettings): Promise<void> => {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      const result = await new Promise<UserSettings | null>(
        (resolve, reject) => {
          request.onsuccess = () => {
  ...
  // 通知条件追加
  const addNotificationCondition = useCallback(
    async (
  ...
  // 通知条件更新
  const updateNotificationCondition = useCallback(
    async (conditionId: string, updates: Partial<NotificationCondition>) => {
      if (!settings) return;

      const updatedConditions = settings.notificationConditions.map(
        (condition) =>
  ...
  // 通知条件削除
  const deleteNotificationCondition = useCallback(
  ...
  // 通知条件の有効/無効切り替え
  const toggleNotificationCondition = useCallback(
    async (conditionId: string, enabled: boolean) => {
